	...
	search_embed: function(embed, callback) {		
		...
		$.getJSON('https://gdata.youtube.com/feeds/api/videos?q=' + embed.key + '&orderby=relevance&v=2&max-results=1&alt=json&category=music', function(data) {			
			...
			resp = {
				...
				code: '<iframe id="youtube-player" class="youtube-player" type="text/html" height="390" src="http://www.youtube.com/embed/' + entry.media$group.yt$videoid.$t + '?enablejsapi=1&origin=http://raibaz.github.com/wrltzr" frameborder="0"></iframe>'
			}
			callback(resp);
		});
	}
