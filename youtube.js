	...
	search_helper: function(embed, max_results, use_first, callback) {
		...
		$.getJSON('https://gdata.youtube.com/feeds/api/videos?q=' + embed.key + '&orderby=relevance&v=2&max-results=' + max_results + '&alt=json&category=music', function(data) {					
			...
			resp = {
				...
				code: '<iframe id="youtube-player" class="youtube-player" type="text/html" height="350" src="http://www.youtube.com/embed/' + entry.media$group.yt$videoid.$t + '?enablejsapi=1&origin=' + escape(window.location.origin) +'" frameborder="0"></iframe>'
			}
			callback(resp);
		});
	}	
